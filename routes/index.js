...
var Router = (function () {
    function Router() {
        ...
        router.get('/profile', this.ensureAuthenticated, function (req, res) {
            ProfileContent.findOne({ userId: req.user._id }, function (error, profile_content) {
                ...
                if (profile_content)
                    text_content = profile_content.content;
        ...
        router.post('/editpage', function (req, res) {
            ProfileContent.update({ userId: req.user._id }, { userId: req.user._id, content: req.body.profile_content }, { upsert: true }, function (err) { });
            res.redirect('/profile');
        });
            ...
            conn.once('open', function () {
                ...
                conn.collection('fs.files').findOne({ "metadata.user": req.params.user_name }, function (err, doc) {
                    if (doc) {
                        ...
                        res.redirect('/images/default.jpg');
                    }
                });
            });
        });
        ...
        router.post('/editcomment/:comic_id', this.ensureAuthenticated, function (req, res) {
            Comic.findOne({ _id: req.params.comic_id }, function (err, comic) {
                ...
                if (!err) {
                    console.log('comic comment changed');
                }
                comic.save(function (err) {
                    if (!err) {
                        console.log('successfully changed comic comment');
                    }
                });
            });
        ...
        router.get('/deletecomic/:comic_id', this.ensureAuthenticated, function (req, res) {
            ...
            conn.once('open', function () {
                    ...
                    if (frames) {
                        frames.forEach(function (frame, index, array) {
                            gfs.remove({ _id: frame._id }, function (err) {
                                if (!err)
                                    console.log('Frame ' + frame._id + ' removed.');
                            });
                        });
                    }
                });
                Comic.findOne({ _id: req.params.comic_id }, function (err, comic) {
                    if (!comic || err)
                        console.log('Error or no comic found with id: ' + req.params.comic_id);
                    comic.remove();
                    res.redirect('/gallery');
                });
            });
                    ...
                    for (var i = 0; i < comic.frames.length; i++) {
                        if (comic.frames[i] == req.params.frame) {
                            ...
                            gfs.remove({ _id: req.params.frame }, function (err) {
                                if (!err)
                                    console.log('Frame ' + req.params.frame + ' removed.');
                            });
                            comic.save(function (err) {
                                if (err)
                                    return;
        ...
        router.post('/privatecomic/:comic_id', ensureAuthenticated, function (req, res) {
            Comic.findOne({ _id: req.params.comic_id }, function (err, comic) {
                comic.private = true;
                if (!err) {
                    console.log('comic privatized');
                }
                comic.save(function (err) {
                    if (!err) {
                        console.log('successfully privatized comic');
                    }
                });
            });
            console.log('hello');
        ...
        router.post('/unpublishcomic/:comic_id', this.ensureAuthenticated, function (req, res) {
            Comic.findOne({ _id: req.params.comic_id }, function (err, comic) {
                ...
                comic.save(function (err) {
                    if (!err) {
                        console.log('successful unpublish');
                    }
                });
            });
            res.redirect('/gallery');
        ...
        router.post('/publiccomic/:comic_id', ensureAuthenticated, function (req, res) {
            Comic.findOne({ _id: req.params.comic_id }, function (err, comic) {
                comic.private = false;
                if (!err) {
                    console.log('comic publicized');
                }
                comic.save(function (err) {
                    if (!err) {
                        console.log('successfully publicized comic');
                    }
                });
            });
            res.redirect('/Workspace/' + req.params.comic_id);
    ...
    Router.prototype.ensureAuthenticated = function (req, res, next) {
        if (req.isAuthenticated()) {
            return next();
        }
        ;
    ...
    Router.prototype.getHost = function (req) {
        return req.get('host');
    };
    return Router;
})();
